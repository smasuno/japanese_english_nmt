...
import sacrebleu
from trainer import Hypothesis, NMT
import numpy as np
...
def train(args: Dict):
    ...
    train_data_src = read_corpus(args['--train-src'], source=vocab_model, vocab_size=vocab_size)      
    ...
    dev_data_src = read_corpus(args['--dev-src'], source=vocab_model, vocab_size=3000)
        ...
        for src_sents, tgt_sents in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            ...
            if train_iter % valid_niter == 0:
                ...
                if is_better:
                    ...
                    torch.save(optimizer.state_dict(), model_save_path + '.optim') # required for Adam to restore the states of the optimizers

                    upload_to_gcs(model_save_path, "gs://japanese_english_nmt/models/model.bin")
                    upload_to_gcs(model_save_path + '.optim', "gs://japanese_english_nmt/models/model.bin.optim")
