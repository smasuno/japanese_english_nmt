...
def pad_sents(sents, pad_token):
    ...
    N = max([len(sentence) for sentence in sents])
    for sentence in sents:
        pad_size = N - len(sentence)
        sents_padded.append(sentence + pad_size * [pad_token])
    ### END CODE HERE
...
def batch_iter(data: list[tuple[list[str], list[str]]], batch_size, shuffle=False):
...
def upload_to_gcs(local_path: str, gcs_uri: str):
    ...
    print(f"Saved model to {gcs_uri}")